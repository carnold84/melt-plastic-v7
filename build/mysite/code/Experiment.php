<?php
/*
 * Experiment DataObjects that behave like page
 * 
 */
class Experiment extends DataObject {
	
	/**
	 * @var defind the listing page class name
	 */
	private static $listing_page_class = 'ExperimentHolder';
	...
	public function getCMSFields() 
	{
		...
		$fields->removeFieldFromTab('Root.Main', 'Url');
		$fields->removeFieldFromTab('Root.Main', 'ExternalUrl');
		$fields->removeFieldFromTab('Root.Main', 'DownloadUrl');
		$fields->removeFieldFromTab('Root.Main', 'Ver');
		...
		$fields->addFieldToTab('Root.Main', new TextField('Url'));
		...
		$fields->addFieldToTab("Root.Main", new UploadField('Thumb', 'Thumbnail image for experiment'));
	...
	{
	  	DataObject::add_extension('Experiment','VersionedExperiment');
		DataObject::add_extension('Experiment',"Versioned('Stage', 'Live')");
	}
	...
	{
		return (DB::query("SELECT \"ID\" FROM \"Experiment_Live\" WHERE \"ID\" = $this->ID")->value())
			? true
			: false;
	}
	...
	{
		$latestPublishedVersion = $this->get_versionnumber_by_stage('Experiment', 'Live', $this->ID);
		$latestVersion = $this->get_versionnumber_by_stage('Experiment', 'Stage', $this->ID);
	...
	public function Link($action = null)
	{
		...
		if($item = Experiment::get()->byID($this->ID))
		{
			//Build link
