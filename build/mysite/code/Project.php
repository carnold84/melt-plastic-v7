<?php
/*
 * Project DataObjects that behave like page
 * 
 */
class Project extends DataObject {
	 ...
	 */
	private static $listing_page_class = 'ProjectHolder';
	...
	{
	  	DataObject::add_extension('Project','VersionedProject');
		DataObject::add_extension('Project',"Versioned('Stage', 'Live')");
	}
	...
	{
		return (DB::query("SELECT \"ID\" FROM \"Project_Live\" WHERE \"ID\" = $this->ID")->value())
			? true
			: false;
	}
	...
	{
		$latestPublishedVersion = $this->get_versionnumber_by_stage('Project', 'Live', $this->ID);
		$latestVersion = $this->get_versionnumber_by_stage('Project', 'Stage', $this->ID);
	...
	public function Link($action = null)
	{
		...
		if($item = Project::get()->byID($this->ID))
		{
			//Build link
